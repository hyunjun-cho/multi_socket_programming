...
static ssize_t get_one_char (connection_t *conn, char *ptr) {
	if (conn->read_cnt <= 0) {
again:
...
ssize_t readline (connection_t *conn, void *vptr, size_t maxlen) {
	int n, rc;
	char c, *ptr;
	ptr = vptr;
	for (n = 1; n < maxlen; n++) {
...
ssize_t writen (connection_t *conn, const void *vptr, size_t n) {
	size_t nleft;
	ssize_t nwritten;
	const char *ptr;
	ptr = vptr;
...
extern void connection_init (connection_t * conn) {
