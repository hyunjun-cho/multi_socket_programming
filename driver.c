...
/* set up IP address of host, using DNS lookup based on SERVERHOST
   ...
   argument list. */
void set_server_address (int argc, char **argv) {
...
void supervisor (supervisor_t *sup) {
...
void alrmhandler (int sig, siginfo_t *info, void *ignored) {
...
void empty_pipe (supervisor_t *sup) {
	int nread, count; 
	char outbuf[MAXLINE];
...
int find_child_id (supervisor_t *children, pid_t pid) {
	int i;
...
void await_children (supervisor_t *children) {
	int status;
	int i;
	pid_t result;
	for ( ; ; ) {
